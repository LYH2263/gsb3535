  ...
  <div class="page-container">
    <div class="view-header">
      <div class="header-content">
        ...
        <p class="view-subtitle">管理全馆图书预约记录与排队队列</p>
      </div>
        ...
        <template v-else>
          <div class="table-card hidden-xs-only">
            <el-table :data="reservations" class="apple-table" row-class-name="apple-table-row">
              ...
              <el-table-column prop="userName" label="预约读者" width="120" />
              <el-table-column label="排队序号" width="100" align="center">
                <template #default="{ row }">
            ...
            <div v-for="item in reservations" :key="item.id" class="reservation-mobile-card">
              <div class="card-header">
                <div class="book-info">
                  ...
                  <p class="user-name">预约人：{{ item.userName }}</p>
                </div>
              ...
              <div class="card-details">
                <div class="detail-row">
                  ...
                  <span v-if="item.status === 'WAITING'" class="value queue-num">第 {{ item.queuePosition }} 位</span>
...
import { Search as IepSearch, Notebook as IepNotebook } from "@element-plus/icons-vue";

const store = useStore();
const loading = computed(() => store.getters["reservations/loading"]);
const reservations = computed(() => store.getters["reservations/allReservations"]);
const keyword = ref("");
...
const handleSearch = () => {
  store.dispatch("reservations/fetchAllReservations", keyword.value);
};

const handleCancel = (row) => {
  ElMessageBox.confirm(`确定要取消《${row.bookTitle}》（预约人：${row.userName}）的预约吗？`, "提示", {
...
onMounted(() => {
  store.dispatch("reservations/fetchAllReservations");
});
</script>
